...
$stmt =$dbconn->prepare("INSERT INTO users(firstname,lastname,email,phone_number,username,hash, hash_id) VALUES(:fname, :lname, :em, :pn, :uname, :h, :hid)");

  $stmt->bindParam(":fname", $input['fname']);
  $stmt->bindParam(":lname", $input['lname']);
  $stmt->bindParam(":em", $input['email']);
  $stmt->bindParam(":pn", $input['pnumber']);
  ...
  $succ = preg_replace('/\s+/', '_', $success);
  header("Location:login?success=$succ");
}
